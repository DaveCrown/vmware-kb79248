...
>- Tower or locally, the `administrator@vsphere.local` password is passed around using an environment variable. While the varible is ephemeral, there is a possibility of it getting leaked. While this is standard operating procedure for Tower, you need to be aware of this.  
>- I'm suggesting you generate and copy ssh keys to your vc's and psc's. Understand the risks and mitigation before you do this. It can be both awesome and terrible. 
...
### Vagrant
Once Vagrant and a hypervisor has been installed, run `vagrant up`. Once the vm is built, run `vagrant ssh` to log into the vm. Once in, `cd /vagrant` to get to the files. To stop it, log out of the vm, and issue the command `vagrant destory`.

## Configuration
You need to define your vcenter environment(s) in the `vcenters.ini` file. It consists of a block for each SSO domain, and all the PSC's and vCenters need to be listed by fdqn. Because the scripts only need to be ran on one server with a PSC role, but all servers in the SSO domain need to restarted in order by role, there is an sts_role setting that need to be set for each server. If the SSO  domain only has a single server, use the `sts_role=all` setting. If the domain is more complex, use the sts_role as shown below. 
### File Format
```ini
[sso_domain]
...
### Run in check mode
#### No ssh keys
`ansible-playbook apply_kb.yml -k` is all you dont have ssh keys. 

`ansible-playbook apply_kb.yml` if you do have 
